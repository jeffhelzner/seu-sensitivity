...
```{python}
#| label: fig-psi-prior
#| fig-cap: "Prior distribution of subjective probabilities over consequences. Left: Distribution of all ψ values from prior samples. Right: Mean subjective probability for each distinct alternative and consequence, averaged over all prior draws."
...
axes[0].hist(all_psi_values, bins=50, density=True, alpha=0.7, 
...
# Right: Heatmap of mean psi for each distinct alternative and consequence
im = axes[1].imshow(mean_psi.T, cmap='YlOrRd', aspect='auto', vmin=0, vmax=1)
axes[1].set_xlabel('Distinct Alternative (r)')
axes[1].set_ylabel('Consequence (k)')
axes[1].set_title('Mean ψ by Alternative and Consequence')
axes[1].set_yticks(range(K))
axes[1].set_yticklabels([f'{k+1}' for k in range(K)])
...
axes[1].set_xticklabels([f'{x+1}' for x in xtick_positions])
plt.colorbar(im, ax=axes[1], label='Mean ψ')
