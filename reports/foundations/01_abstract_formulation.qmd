...
This report establishes the theoretical foundations for our computational model of decision-making agents. We derive three properties of the softmax choice model with respect to an arbitrary value function, then show how these properties apply specifically when values are subjective expected utilities (SEU). While mathematically straightforward, these properties are essential to our intended interpretation: when value is taken to be subjective expected utility, $\alpha$ can be interpreted as measuring a decision maker's alignment with the normative standard of SEU rationality—how consistently their choices track the SEU ranking.
...
1. **Subjective probabilities** $\boldsymbol{\psi}_r \in \Delta^{K-1}$ over $K$ consequences
2. **An expected utility** computed as:
   $$
...
2. **Normative content**: SEU maximization is a rationality criterion—Properties 1-3 characterize adherence to this normative standard [@savage1954]

3. **Empirical predictions**: The model predicts that choices will track $\eta$, providing testable restrictions
...
### Resolution: Utility Standardization
...
### Interpretation of α Under Standardization
...
#| tbl-cap: "Interpretation of α for a one-unit expected utility difference (maximum possible difference with standardized utilities)."
...
**Not a cognitive process model.** We make no claim about how decision makers actually deliberate. The model is silent on whether agents compute probabilities, form expectations, or engage in any particular mental procedure. It specifies a distribution over choices, not a mechanism that generates them.

**Not bounded rationality.** Bounded rationality programs, in their various formulations [@simon1955; @gigerenzer1996], typically propose alternative decision procedures—heuristics, satisficing rules, or fast-and-frugal strategies—that agents use in place of optimization. Our model posits no such alternative procedures. Nor do we invoke notions of ecological rationality or fit between heuristics and environmental structure. The model simply describes a stochastic relationship between SEU values and choice probabilities.

### A Conceptual Lens: Commitment and Performance

A useful conceptual lens comes from Isaac Levi's distinction between **commitment** and **performance** [@levi1980]. We may be committed to standards we fail to perform up to. Most of us are committed to the laws of arithmetic despite occasionally making calculation errors; the errors are failures of performance, not rejections of the standard.
...
However, our framework differs from Levi's in important ways. Levi was not an "orthodox Bayesian"—he made important contributions to theories of indeterminate probability and to decision theories that relax the complete ordering assumption of SEU [@levi1980]. He also suggested that the commitment-performance gap could be reduced through tools (e.g., calculators reduce arithmetic errors).

In our framework, by contrast, there is a residual stochasticity that persists even as $\alpha \to \infty$. The softmax rule assigns positive probability to all alternatives for any finite $\alpha$, and even in the limit, the model describes a *distribution* over choices anchored at log-odds of zero. Observable departures from SEU maximization can occur even for decision makers with arbitrarily high sensitivity. This places a constraint on how far the commitment-performance gap can be closed that Levi's framework does not obviously share. We borrow his conceptual vocabulary while operating in a rather different modeling tradition.

### The "As-If" Character of the Model

We emphasize the model's status as an **as-if** description of choice behavior. The generative process specified—computing expected utilities, then selecting via softmax—need not correspond to any actual cognitive procedure. What matters is that the *distribution over choices* implied by the model matches empirical choice frequencies.

This as-if stance permits agnosticism about cognitive mechanisms while enabling precise quantitative predictions. The sensitivity parameter $\alpha$ is estimated from choice data, not introspected or measured via reaction times.

However, we do not take a purely instrumentalist stance focused only on prediction. The goal of our framework is interpretation: by fitting the model and recovering a posterior distribution over $\alpha$, we aim to characterize the decision maker's sensitivity to SEU differences in a theoretically meaningful way.
