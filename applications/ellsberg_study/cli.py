"""
Command-line interface for the Ellsberg Study.

Usage:
    python -m applications.ellsberg_study run [options]
    python -m applications.ellsberg_study estimate-cost [options]
    python -m applications.ellsberg_study validate [options]
"""
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    if alt_path.exists():
        ...
        descriptions = [a["description"] for a in alts]
...
def cmd_estimate_cost(args: argparse.Namespace) -> None:
    ...
    # If model not in pricing table, try a reasonable default for Claude 3.5 Sonnet
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    config = StudyConfig.from_yaml(args.config)
    runner = EllsbergStudyRunner(config)
    summary = runner.run(
...
def cmd_fit(args: argparse.Namespace) -> None:
    ...
    # Allow overriding stan_model from CLI
    if args.model:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="ellsberg_study",
        description="Ellsberg Study: LLM sensitivity x temperature (Ellsberg urn gambles)",
    )
    parser.add_argument(
    ...
    p_fit.add_argument(
        ...
        help="Stan model name (e.g. m_0, m_02). Overrides config.",
    )
