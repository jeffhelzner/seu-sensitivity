/**
 * Bayesian Decision Theory Model (m_03)
 *
 * Structurally identical to m_0.  Differs by exposing the
 * Dirichlet concentration on the utility-difference simplex
 * delta as a data input, supporting Route 2 from Report 4
 * (sharpening (beta, delta) recovery via a more concentrated
 * prior on delta).
 *
 * Prior:  alpha ~ lognormal(0, 1)             (as in m_0)
 *         beta  ~ std_normal()                (as in m_0)
 *         delta ~ dirichlet(delta_concentration * 1_{K-1})
 *
 *   delta_concentration = 1   -> flat (= m_0)
 *   delta_concentration > 1   -> tighter around 1/(K-1)
 *
 * See reports/foundations/13_concentrated_delta_prior.qmd
 * for the Route 2 evaluation.
 */
data{
  ...
  real<lower=0> delta_concentration; // Dirichlet concentration scalar on delta (1 = flat / m_0 prior)
}
transformed data {
...
model{
  ...
  delta ~ dirichlet(rep_vector(delta_concentration, K-1)); // Concentrated Dirichlet on delta
