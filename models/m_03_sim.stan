/**
 * Data Simulation Model for m_03
 *
 * Structurally identical to m_0_sim.  Differs by exposing the
 * Dirichlet concentration on delta as a data input so the
 * data-generating prior can be matched to the inference prior
 * of m_03.stan.
 *
 *   delta_concentration = 1   -> flat (= m_0_sim)
...
data {
  ...
  real<lower=0> alpha_mean;       // mean for alpha parameter (default=0 for lognormal)
  real<lower=0> alpha_sd;         // sd for alpha parameter (default=1 for lognormal)
  real<lower=0> beta_sd;          // sd for beta coefficients (default=1)
  real<lower=0> delta_concentration; // Dirichlet concentration scalar on delta
}
...
generated quantities {
  ...
  // Generate utility differences using a concentrated Dirichlet prior
  simplex[K-1] delta = dirichlet_rng(rep_vector(delta_concentration, K-1));
