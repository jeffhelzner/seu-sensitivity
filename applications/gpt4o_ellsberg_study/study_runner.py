"""
Study Runner for the GPT-4o × Ellsberg Study (Cell 1,2).

Thin wrapper around the Ellsberg study pipeline, using this module's
config (OpenAI provider, GPT-4o temperature grid).
"""
...
# Reuse all task-level components from ellsberg study
from applications.ellsberg_study.problem_generator import ProblemGenerator
from applications.ellsberg_study.assessment_collector import AssessmentCollector
from applications.ellsberg_study.choice_collector import ChoiceCollector
...
class GPT4oEllsbergStudyRunner:
    """
    End-to-end pipeline for the GPT-4o × Ellsberg study.

    Same 4-phase structure as EllsbergStudyRunner but configured
    for OpenAI GPT-4o on the Ellsberg urn gambles task.
    """
    ...
    def run(
        ...
        *,
        ...
        if not skip_collection:
            ...
            per_temp_choices = self._run_phase2b(problems, per_temp_assess)
    ...
    def _run_phase3(
        ...
        builder = EllsbergStanDataBuilder(self.config)
        ...
        for temp in self.config.temperatures:
            ...
            issues = EllsbergStanDataBuilder.validate_stan_data(stan_data)
            if issues:
                for issue in issues:
    ...
    def _run_phase4(
        ...
        for temp_str, info in phase3_output.get("per_temperature", {}).items():
            ...
            np.savez_compressed(fit_dir / "alpha_draws.npz", alpha=alpha_draws)
            ...
            logger.info(
                "  T=%.1f: alpha mean=%.3f, median=%.3f, 90%% CI=[%.3f, %.3f]",
                temp,
        ...
        summary_out = {
            temp_str: {k: v for k, v in info.items() if k != "diagnostics"}
