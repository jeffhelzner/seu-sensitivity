/**
 * Bayesian Decision Theory Model (m_2) for Simulation Based Calibration
 * 
 * Extends m_1 SBC with separate sensitivity parameters:
...
transformed data {
  ...
  real<lower=0> omega_ = lognormal_rng(0, 1);
  ...
  // Generate choices for risky problems (using omega_)
...
model {
  ...
  // Likelihood
  {
    ...
    // Likelihood for risky choices (using omega)
    {
      int pos = 1;
      for (n in 1:N) {
...
generated quantities {
  ...
  // Parameters: alpha, omega, beta (K*D), delta (K-1)
