/**
 * Bayesian Decision Theory Model (m_03) for Simulation Based Calibration
 *
 * Structurally identical to m_0_sbc.  Differs by exposing the
 * Dirichlet concentration on delta as a data input.  Crucially,
 * delta_concentration is used in BOTH the true-parameter draw
 * (delta_) AND the inference prior (delta), maintaining the
 * matched-prior contract that SBC requires.
 *
 *   delta_concentration = 1   -> flat (= m_0_sbc)
...
transformed data {
  ...
  simplex[K-1] delta_ = dirichlet_rng(rep_vector(delta_concentration, K-1));
...
model {
  ...
  delta ~ dirichlet(rep_vector(delta_concentration, K-1));
