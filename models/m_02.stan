/**
 * Bayesian Decision Theory Model (m_02)
 *
 * Structurally identical to m_0.  Differs only in the prior
 * hyperparameters for alpha, which are chosen via prior predictive
 * analysis for the Ellsberg study (K=4 monetary consequences).
 *
 * Prior:  alpha ~ lognormal(3.5, 0.75)
 *   median  ≈ 30,  90 % CI ≈ [10, 124]
 *   prior-implied SEU-max rate ≈ 0.76  (K=4)
 *
 * See scripts/run_prior_predictive_grid.py and
 * results/prior_predictive/grid_search_ellsberg/ for the calibration rationale.
 */
data{
...
model{
  // Priors
  alpha ~ lognormal(3.5, 0.75);     // Informative prior on choice sensitivity (calibrated via prior predictive analysis, K=4 Ellsberg)
