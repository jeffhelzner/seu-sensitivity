/**
 * Bayesian Decision Theory Model (m_21)
 *
 * Structurally identical to m_2.  Differs only in the prior
 * hyperparameters for alpha and omega, which are chosen via prior
 * predictive analysis rather than convenience defaults.
 *
 * Priors:
 *   alpha ~ lognormal(3.0, 0.75)   [uncertain sensitivity]
 *     median ≈ 20,  90 % CI ≈ [5, 73]
 *   omega ~ lognormal(3.0, 0.75)   [risky sensitivity]
 *     median ≈ 20,  90 % CI ≈ [5, 73]
 *   prior-implied SEU-max rate ≈ 0.77 (uncertain), 0.85 (risky)
 *
 * See scripts/run_prior_predictive_grid_augmented.py and
 * results/prior_predictive/augmented_grid_search/m_2/ for calibration.
 */
data {
...
parameters {
  ...
  real<lower=0> omega;           // sensitivity parameter for risky choices
...
transformed parameters {
  ...
  // Construct choice probabilities for risky problems (using omega)
  {
    int pos = 1;
    for (n in 1:N) {
...
model {
  ...
  omega ~ lognormal(3.0, 0.75);     // Calibrated via prior predictive analysis
