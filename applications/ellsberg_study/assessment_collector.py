"""
Assessment Collector for the Ellsberg Study.

For each alternative in the pool, sends an assessment prompt at the specified
temperature, records the text response, and embeds it. One assessment
per (alternative, temperature) — not per (problem, alternative) — so that the
...
class AssessmentCollector:
    """
    Collect per-alternative assessment texts and embeddings at each temperature.

    Iterates over *alternatives* (not problem x alternative), producing exactly
    one assessment and one embedding per alternative per temperature.
    """
    ...
    def collect_temperature(
        ...
        """
        Collect assessments for all alternatives at a single temperature.

        Returns:
            (assessments_dict, raw_embeddings)
        """
        ...
        for alt in self.alternatives:
            ...
            raw_embeddings[aid] = np.array(embedding_vec)
    ...
    def collect_all_temperatures(
        ...
        Returns:
            (per_temp_assessments, per_temp_raw_embeddings)
        """
    ...
    def get_assessment_texts(
        ...
        """Extract a mapping of alternative_id -> assessment text."""
        return {
            entry["alternative_id"]: entry["response"]
