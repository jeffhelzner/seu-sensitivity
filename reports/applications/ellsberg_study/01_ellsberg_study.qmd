...
description: |
  ...
  in the initial temperature study (GPT-4o, insurance triage) generalizes
...
The initial temperature study ([Report 1](../temperature_study/01_initial_study.qmd)) established a clear negative relationship between LLM sampling temperature and estimated sensitivity $\alpha$ to subjective expected utility maximization. Higher temperature yielded lower $\alpha$, with a posterior slope $\Delta\alpha / \Delta T \approx -25$ and $P(\text{slope} < 0) > 0.99$. That study used **GPT-4o** (OpenAI) on an **insurance claims triage** task with $K = 3$ consequences.

A natural question is whether this finding generalizes: does it depend on the specific task domain, the specific LLM, or both? Answering this question requires a $2 \times 2$ factorial design crossing LLM (GPT-4o vs. Claude 3.5 Sonnet) with task domain (insurance triage vs. Ellsberg gambles). This report describes one cell of that design — **Claude 3.5 Sonnet on Ellsberg gambles** — which changed both factors simultaneously relative to the initial study:

- **Task domain**: Ellsberg-style urn gambles with monetary payoffs instead of insurance claims triage. The consequence space expands to $K = 4$ ($\$0, \$1, \$2, \$3$). Alternatives range from fully-specified (known ball counts) to maximally ambiguous (unknown mixture), inspired by @ellsberg1961's seminal paradox. The classic Ellsberg finding is that human decision-makers prefer known-probability gambles over ambiguous ones, violating the Sure-Thing Principle of @savage1954's subjective expected utility theory — a pattern known as *ambiguity aversion* [@gilboa1989; @machina2014]. Whether LLMs exhibit an analogue of this bias is an open question that this task domain can begin to address.
...
::: {.callout-important}
## Summary of Findings
The monotonic temperature–α relationship observed in the initial temperature study was **not replicated** in this study. The posterior slope is $\Delta\alpha / \Delta T \approx -19$ but with substantial uncertainty ($P(\text{slope} < 0) \approx 0.77$), and the per-temperature $\alpha$ estimates exhibit a non-monotonic pattern. The model fits adequately at every temperature level — the non-replication reflects the behavior of the data, not a modeling artifact. Because this study changed both the task and the LLM simultaneously, the non-replication cannot be attributed to either factor alone, nor can it rule out an interaction between them. This analysis pools across the three ambiguity tiers of the Ellsberg gamble set and therefore addresses overall SEU sensitivity rather than ambiguity aversion or tier-specific processing (see Discussion). Cross-cell attribution — main effects of LLM and task, and any interaction — is addressed in the [Factorial Synthesis](../factorial_synthesis/01_factorial_synthesis.qmd).
:::
...
We use Ellsberg-style urn gambles in which Claude 3.5 Sonnet selects which gamble to play from a set of alternatives. Each gamble describes an urn containing colored balls with specified (or partially specified) counts, and payout rules mapping ball colors to one of $K = 4$ monetary consequences ($\$0, \$1, \$2, \$3$). In each decision problem, the LLM is presented with a subset of these gambles. The LLM first *assesses* each gamble individually (producing text that is then embedded), and subsequently makes a *choice* among the gambles in a given problem.
...
The alternative pool consists of 30 Ellsberg-style urn gambles organized into three ambiguity tiers:
...
- **Tier 3 (high ambiguity, E21–E30):** Only one color's count is known; the rest is described as an unknown mixture. This is closest to Ellsberg's original setup.
...
- **Tier 3 example (E21):** An urn contains 90 balls — exactly 30 are red; the remaining 60 are an unknown mixture of black and yellow balls. Drawing red pays \$3, black pays \$1, yellow pays \$0. This is closest to Ellsberg's original setup: one color's probability is known, but the rest are genuinely ambiguous.

The tiered structure provides a systematic gradient from risk (known probabilities) to Knightian uncertainty (unknown probabilities), connecting naturally to @ellsberg1961's original paradigm. Importantly, the current analysis applies a single SEU model to all alternatives pooled across tiers. This is appropriate as an overall sensitivity measure, but it means the model assumes point-valued subjective probabilities even for ambiguous alternatives — an assumption that is violated if the LLM exhibits ambiguity aversion. See @sec-discussion for further discussion of this limitation.
...
Across all conditions, the distribution of choices across ordinal positions (first-listed, second-listed, etc.) was examined and the proportion of choices selecting the first-listed alternative was consistent with what would be expected given the varying number of alternatives per problem (we did not detect a systematic position bias in this descriptive check). The counterbalancing protocol (three shuffled presentations per base problem) is designed to make any residual position effects approximately orthogonal to gamble identity, substantially reducing the risk that position confounds $\alpha$ estimation.
:::
...
Pooling embeddings across temperatures before PCA means the resulting basis reflects a mixture of temperature-dependent and gamble-specific variation. Since the embedding model (`text-embedding-3-small`) is deterministic and operates on Claude's assessment text — which may vary in length and phrasing at higher temperatures — some temperature-induced embedding shift is possible. The PCA variance summary below shows that the dominant components capture a substantial share of variance (the first component alone explains 27%, and 32 components retain 87.8% of the variance, comparable to the initial study). We do not separately decompose the variance into gamble-level and temperature-level components in this report; the interpretation that the leading components primarily reflect gamble-level structure is consistent with the design (the assessments describe the same underlying urn structure regardless of temperature) but is not formally established here.
...
The model uses the following key parameters (see the [foundations reports](../foundations/) for complete derivations):
...
The m_02 prior is slightly wider than m_01's because the larger $K = 4$ consequence space spreads expected utilities across alternatives differently, flattening the softmax response so that higher $\alpha$ values are needed to achieve comparable SEU-maximization rates. (This is *not* because the random-choice baseline changes: the menu size $N_m \in \{2, 3, 4\}$ is the same in both studies, so a uniform-random chooser selects the SEU-maximizer at the same rate regardless of $K$.)
...
The selected prior $\text{Lognormal}(3.5, 0.75)$ yields a prior-implied SEU-max rate of approximately 0.76 for $K = 4$, comparable to the m_01 prior's 0.78 rate for $K = 3$. The calibration target was to match the prior-implied SEU-maximization rate within 5 percentage points of the initial study's rate, ensuring that the prior encodes a similar degree of informativeness about the LLM's decision quality across the two studies. Alternative calibration criteria (e.g., matching prior-implied choice entropy or prior probability mass in a specific α range) were not systematically explored; the SEU-max rate was chosen for its interpretability as a baseline measure of decision quality.
...
#| fig-cap: "Recovery of the sensitivity parameter α under the m_02 prior with the Ellsberg study design (K=4). Left: true vs. estimated values with identity line. Right: 90% credible intervals for each iteration, colored by whether they contain the true value."
...
Posterior predictive checks (reported below) are satisfactory across all five conditions, and effective sample size, treedepth, and E-BFMI diagnostics are clean. Three of the five fits also produce a small number of divergent transitions at the lower temperatures (4/4000 at $T = 0.0$, 5/4000 at $T = 0.2$, and 2/4000 at $T = 0.5$, each $< 0.15\%$); these are well within acceptable bounds, and the α posteriors themselves remain well-mixed. R-hat is satisfactory for all parameters in every condition.
...
All posterior predictive p-values fall within $[0.3, 0.6]$, indicating that the model provides an adequate description of the choice data at every temperature level. The non-monotonic pattern in α is not an artifact of model misfit—the m_02 model is fitting the data well at each temperature.
...
## Within-LLM Comparison Across Tasks (Claude) {#sec-comparison}

To isolate the effect of task domain while holding the LLM fixed, we compare this study (Claude × Ellsberg) with the companion [Claude × Insurance study](../claude_insurance_study/01_claude_insurance_study.qmd), which uses the same Claude 3.5 Sonnet model and the same temperature grid $\{0.0, 0.2, 0.5, 0.8, 1.0\}$. The two studies differ only in the task: Ellsberg urn gambles ($K = 4$, m_02) here versus insurance triage ($K = 3$, m_01) there. Because model variant, prior, and $K$ differ, absolute α magnitudes are not directly comparable across panels; the qualitative comparison concerns the *shape* of the temperature–α relationship within Claude.

```{python}
#| label: fig-cross-study
#| fig-cap: "Within-Claude cross-task comparison of posterior α estimates by temperature. Left: Claude × Insurance (K=3, m_01). Right: Claude × Ellsberg, this study (K=4, m_02). Both studies use the same Claude 3.5 Sonnet model and the same temperature grid; α magnitudes are not directly comparable across panels because K, model variant, and prior differ. Error bars show 90% credible intervals."

# Load Claude × Insurance companion-study data
companion_data_dir = Path("..") / "claude_insurance_study" / "data"

with open(companion_data_dir / "primary_analysis.json") as f:
    companion_analysis = json.load(f)

companion_temps = [s['temperature'] for s in companion_analysis['summary_table']]
companion_medians = [s['median'] for s in companion_analysis['summary_table']]
companion_lows = [s['ci_low'] for s in companion_analysis['summary_table']]
companion_highs = [s['ci_high'] for s in companion_analysis['summary_table']]
...
# Left: Claude × Insurance
ax = axes[0]
ax.errorbar(companion_temps, companion_medians,
            yerr=[np.array(companion_medians) - np.array(companion_lows),
                  np.array(companion_highs) - np.array(companion_medians)],
            fmt='o-', color=SEU_COLORS['primary'], linewidth=2, markersize=8,
            capsize=5, capthick=1.5)
ax.set_xlabel('Temperature')
ax.set_ylabel('Sensitivity (α)')
ax.set_title('Claude × Insurance\n(K=3, m_01)')
ax.set_xticks(companion_temps)

# Right: Claude × Ellsberg (this study)
ax = axes[1]
...
ax.set_xlabel('Temperature')
ax.set_ylabel('Sensitivity (α)')
ax.set_title('Claude × Ellsberg (this study)\n(K=4, m_02)')
...
#| tbl-cap: "Within-Claude cross-task comparison of slope estimates and monotonicity. Both cells use Claude 3.5 Sonnet on the same temperature grid; neither shows strong evidence of a monotonic temperature–α relationship."

cross = pd.DataFrame([
    {'Study': 'Claude × Insurance (K=3, m_01)',
     'Slope median': f"{companion_analysis['slope']['median']:.1f}",
     'Slope 90% CI': f"[{companion_analysis['slope']['ci_low']:.1f}, {companion_analysis['slope']['ci_high']:.1f}]",
     'P(slope < 0)': f"{companion_analysis['slope']['p_negative']:.3f}",
     'P(strict mono)': f"{companion_analysis['monotonicity_prob']:.3f}"},
    {'Study': 'Claude × Ellsberg (K=4, m_02, this)',
...
Holding the LLM fixed, neither Claude cell shows strong evidence of a monotonic temperature–α relationship: the Insurance slope is centered near zero with $P(\text{slope} < 0) \approx 0.56$, and the Ellsberg slope is weakly negative with $P(\text{slope} < 0) \approx 0.77$. The non-replication observed here therefore does not appear to be task-specific within Claude. The full 2×2 comparison — including the GPT-4o cells, formal main-effects analyses, and the interaction — is reported in the [Factorial Synthesis](../factorial_synthesis/01_factorial_synthesis.qmd).
...
4. **Model adequacy.** Posterior predictive checks show no evidence of misfit at any temperature level. The non-replication reflects the behavior of the data, not a modeling artifact.
...
3. **Narrower temperature range.** The Anthropic API limits temperature to $[0, 1]$, compared to OpenAI's $[0, 2]$. The initial study's strongest signal came from the separation between $T \leq 0.7$ and $T \geq 1.0$. The Ellsberg study cannot probe the high-temperature regime ($T > 1.0$) at all. As discussed in the design section, the expected effect under the initial study's slope would be approximately $-25$ over $[0, 1.0]$ versus $-38$ over $[0, 1.5]$.

4. **Ambiguity effects.** The Ellsberg gambles include alternatives with varying levels of ambiguity. If Claude processes ambiguous alternatives differently at different temperatures — e.g., becoming more or less ambiguity-averse as temperature changes — this could create non-monotonic patterns in overall α. The classic finding in human decision-making is systematic ambiguity aversion: people prefer known-probability gambles over ambiguous ones, even when the expected values are comparable [@ellsberg1961; @camerer1992]. Whether LLMs exhibit an analogue of this pattern, and whether it varies with temperature, is an open question. The current model assumes SEU (point-valued subjective probabilities), which cannot capture ambiguity aversion directly. A model with set-valued probabilities or separate ambiguity-attitude parameters (e.g., α-MEU; @ghirardato2004) would be needed to formally test this hypothesis.

5. **LLM × task interaction.** The non-replication need not be attributable to either factor alone. A genuine interaction — where the effect of temperature on α depends on the combination of model and task — would produce a pattern indistinguishable from the main-effect explanations above, given only this single cell.
...
### Connection to Human Decision-Making Literature

The use of a softmax sensitivity parameter $\alpha$ connects to a rich tradition of stochastic choice models in human decision-making research. @hey1994 introduced explicit noise parameters in experimental economics to account for the imperfect consistency of human choices; the α parameter here plays an analogous role, measuring how consistently choices align with the fitted utility structure. In reinforcement learning and behavioral economics, the inverse-temperature parameter in softmax action selection is a standard model of the exploration–exploitation trade-off [@sutton2018], and trembling-hand equilibria in game theory [@selten1975] model choice noise as small perturbations from optimal play.

More recently, rational inattention models [@caplin2015; @matejka2015] provide an information-theoretic foundation: agents with limited processing capacity optimally introduce choice noise proportional to the cost of acquiring decision-relevant information. Under this interpretation, temperature-induced variation in $\alpha$ could reflect how the sampling procedure affects the LLM's effective information-processing capacity — a computational analogue of cognitive load effects documented in human studies [@deck2015]. The non-replication reported here suggests that this mapping from temperature to decision noise is not straightforward and may depend on the interaction between model architecture and task structure.
...
These two additional conditions complete the factorial and allow attribution of any temperature–α effect to LLM, task, or their interaction. The results are reported in Reports 5 (Claude × Insurance) and 6 (GPT-4o × Ellsberg), with the full factorial analysis in Report 7 ([Factorial Synthesis](../factorial_synthesis/01_factorial_synthesis.qmd)); the cross-cell interpretation belongs to that report.
...
## Appendix: SBC Results for m_02 {#sec-appendix-sbc}
...
```{python}
#| label: fig-sbc-alpha
#| fig-cap: "SBC rank histogram for α (75 iterations). Uniform distribution indicates proper calibration. The dashed red lines show the 95% confidence band for a uniform distribution. The α ranks show no systematic deviation from uniformity (χ² p = 0.18)."
