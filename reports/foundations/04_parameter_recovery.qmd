...
- **δ (utility increments)** shows weaker recovery, with wider credible intervals and lower precision

This pattern suggests that the utility parameters are harder to learn from decisions under uncertainty, though they do respond to the data.
:::

To understand why δ is harder to recover, consider the structure of the model. In decisions under uncertainty, choices depend on **expected utilities**:
...
Different utility vectors can produce similar expected utility rankings when combined with appropriately adjusted subjective probabilities. This makes δ harder to pin down precisely:

- β and α affect how *sensitively* choices respond to expected utility differences
- δ affects the *scale* of those differences
- The data provides weaker constraints on δ than on the other parameters

Note that this does not mean δ is completely uninformed by the data—the posteriors do concentrate relative to the prior—but the information accumulates more slowly than for α and β.

### Does Increasing Sample Size Help?

A natural question is whether the weaker δ recovery is simply a matter of insufficient data. The study design with M=25 decision problems might not provide enough information. Let's test this by doubling the sample size to M=50.
...
The M=50 extension above adds problems over the **same** alternatives—it doesn't expand the feature space. But we might hypothesize that adding **new** alternatives (with new feature vectors) provides qualitatively different information that could help with δ recovery.
...
#| fig-cap: "Three-way comparison of δ recovery: M=25 (base), M=50 (same alternatives), and M=50 (new alternatives). Adding new alternatives may provide different information than simply adding more problems over existing alternatives."
...
three_way_data.append({
    'Parameter': 'α',
    'M=25 RMSE': f'{alpha_rmse:.4f}',
    'M=50 (same) RMSE': f'{alpha_rmse_m50:.4f}',
    'M=50 (new) RMSE': f'{alpha_rmse_m50_new:.4f}',
    'M=25 CI': f'{alpha_ci_width:.3f}',
    'M=50 (same) CI': f'{alpha_ci_width_m50:.3f}',
    'M=50 (new) CI': f'{alpha_ci_width_m50_new:.3f}'
})
...
for i in range(K_minus_1):
    three_way_data.append({
        'Parameter': f'δ_{i+1}',
        'M=25 RMSE': f'{delta_df.iloc[i]["rmse"]:.4f}',
        'M=50 (same) RMSE': f'{delta_df_m50.iloc[i]["rmse"]:.4f}',
        'M=50 (new) RMSE': f'{delta_stats_m50_new[i]["rmse"]:.4f}',
        'M=25 CI': f'{delta_df.iloc[i]["ci_width"]:.3f}',
        'M=50 (same) CI': f'{delta_df_m50.iloc[i]["ci_width"]:.3f}',
        'M=50 (new) CI': f'{delta_stats_m50_new[i]["ci_width"]:.3f}'
    })
...
The three-way comparison reveals important patterns about what helps δ recovery:
...
The asymmetry in learning rates for δ likely reflects the model structure: in decisions under uncertainty, utilities and subjective probabilities enter the likelihood through their product (expected utilities). This creates a partial confounding that makes δ harder to pin down than parameters like α that more directly govern choice sensitivity.
...
The effectiveness of these changes for δ recovery is an empirical question worth exploring in future work.
...
This provides an alternative route to improving δ recovery that may be more efficient than simply increasing M. See [Report 5](05_adding_risky_choices.qmd) for the full development and comparison.
:::
...
1. **Well-recovered parameters**: α (sensitivity) and β (feature weights) can be recovered with good precision
2. **Slower-learning parameters**: δ (utility increments) shows wider uncertainty, though it does respond to data
3. **Sample size helps, but differentially**: Doubling M improves α more than δ

This pattern reflects the structure of the SEU model: utilities and subjective probabilities enter the likelihood through their product (expected utilities), making them harder to separate than parameters like α that more directly govern choice behavior.

**Two paths forward**:

1. **More data**: Larger M (and perhaps R) may eventually yield precise δ estimates, though the rate of improvement appears slower than for other parameters
2. **Different data**: Adding risky choices (model m_1) provides a complementary data source that may help constrain utilities more directly
