"""
Configuration for the GPT-4o × Ellsberg Study (Cell 1,2).

Overrides Ellsberg study defaults for OpenAI provider and
GPT-4o temperature grid.
"""
...
# Default paths: reuse ellsberg study's alternatives and prompts
_MODULE_DIR = Path(__file__).resolve().parent
_ELLSBERG_STUDY_DIR = _MODULE_DIR.parent / "ellsberg_study"
DEFAULT_CONFIG_PATH = _MODULE_DIR / "configs" / "study_config.yaml"
DEFAULT_PROMPTS_PATH = _ELLSBERG_STUDY_DIR / "configs" / "prompts.yaml"
DEFAULT_ALTERNATIVES_PATH = _ELLSBERG_STUDY_DIR / "configs" / "alternatives.json"
...
class StudyConfig:
    """
    Configuration for the GPT-4o × Ellsberg Study.

    Same task as ellsberg_study (Ellsberg urn gambles, K=4) but uses
    OpenAI GPT-4o with its temperature grid [0.0, 0.3, 0.7, 1.0, 1.5].
    """

    # Temperature conditions (OpenAI range)
    ...
    def validate(self) -> List[str]:
        ...
        if self.K != 4:
            raise ConfigError("K must be 4 for the Ellsberg task")
        if self.target_dim < 1:
            raise ConfigError("target_dim must be >= 1")
        if self.provider not in ("openai", "anthropic"):
            raise ConfigError(f"Unknown provider: {self.provider}")

        # No Anthropic temperature cap — GPT-4o supports T > 1.0
