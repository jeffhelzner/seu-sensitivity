...
We do not pursue this route empirically in the body of this report; [Report 13](13_concentrated_delta_prior.qmd) evaluates it directly, using a parameterized model `m_03` that exposes the Dirichlet concentration as a data input and runs a matched sim+inference sweep across $\alpha_0 \in \{1, 2, 5, 10\}$.
...
3. **A more concentrated Dirichlet prior on δ**: imports prior information rather than data; cheap but assumption-laden, and only as defensible as the substantive grounds for the tighter prior. Evaluated empirically in [Report 13](13_concentrated_delta_prior.qmd).
