...
The comparison reveals differential effects of increased sample size:

::: {.callout-note}
## Sample Size Effects Vary by Parameter

Doubling the sample size from M=25 to M=50:

- **α recovery improves**: RMSE decreases, CI widths narrow noticeably
- **δ recovery shows modest improvement**: Some reduction in RMSE and CI width, but less dramatic than for α

This suggests that δ parameters *are* informed by the data—they're not completely unidentified—but the information accumulates more slowly. Whether δ would achieve the same precision as α with sufficiently large M is an open question that would require larger-scale simulations to answer definitively.
:::

The asymmetry in learning rates likely reflects the model structure: in decisions under uncertainty, utilities and subjective probabilities enter the likelihood through their product (expected utilities). This creates a partial confounding that makes δ harder to pin down than parameters like α that more directly govern choice sensitivity.
