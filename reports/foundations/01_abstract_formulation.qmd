...
### Property 3: Uniform Choice Limit {#sec-random}
...
This non-uniqueness has historical resonance with Luce's [@luce1959] derivation of the softmax functional form: Luce worked with abstract "scale values" determined only up to a common multiplicative constant, foreshadowing the same kind of indeterminacy we encounter when interpreting $\alpha$ against an unfixed utility scale.
...
The intended interpretation of our model is of a decision maker who is committed to SEU maximization but has limited sensitivity to the implications of that commitment. The term "commitment" is doing substantial work here, and we elaborate it in the [Conceptual Lens](#a-conceptual-lens-commitment-and-performance) subsection below. The sensitivity parameter $\alpha$ measures how reliably the agent's choices track the SEU ranking:
...
### What we do not intend the model to describe
...
3. **Uniform choice limit**: As $\alpha \to 0$, choices become uniformly distributed over the available alternatives
