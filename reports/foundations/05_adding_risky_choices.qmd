...
[Report 4](04_parameter_recovery.qmd) revealed differential parameter recovery in model m_0: the sensitivity parameter α is recovered well, while both the feature weights β and the utility increments δ show materially weaker precision because they enter the expected utility function jointly through the belief–utility decomposition. The CI-width diagnostics in Report 4 nonetheless provide evidence that δ is updated by the data relative to its broad prior support—but the information accumulates more slowly than for α. (For the K=3 design used throughout, the Dirichlet(1,1) prior gives each δ component a uniform marginal on [0,1], so a central 90% prior interval has width ≈ 0.90; the observed posterior CI widths in Report 4 sit modestly below this and shrink further in richer designs.)

One approach to improving δ recovery is simply to collect more data. However, the decision-theoretic distinction between risk and uncertainty suggests another possibility: incorporating **risky decisions** where probabilities are known to the decision-maker. We focus here on δ because risky choices provide a particularly direct way to constrain utilities: their expected utilities depend on the utility vector directly, without requiring the model to infer subjective probabilities from features at the same time.
...
This framework provides useful historical context for model m_1. By observing choices in both:
...
...we may obtain separate sources of information about utilities and subjective probabilities.
...
Model m_1 uses a related empirical strategy. The connection is conceptual rather than a direct application of the Anscombe-Aumann representation theorem: here we are asking whether adding known-probability choice data improves recovery of the parameters in a specific statistical model.
...
::: {.callout-note}
## Notation
We use $N$ for the number of risky decision problems in model m_1. This is distinct from any per-problem alternative counts used elsewhere in the series; context always disambiguates. In the Stan implementation, the probability vectors denoted $\boldsymbol{\pi}_s$ in the mathematical exposition correspond to the data array `x[s]` (declared as `simplex[K]`), and the feature vectors $\mathbf{w}_r$ correspond to `w[r]` (declared as `vector[D]`).
:::
...
The key insight: risky expected utilities depend *only* on υ (and hence δ), not on β. This reduces the structural coupling between utility and subjective-probability parameters that makes δ harder to learn in m_0.
...
::: {.callout-warning}
## Assumption: Shared Sensitivity Parameter
Model m_1 assumes the **same** sensitivity parameter α governs both risky and uncertain choices. This is a substantive behavioral assumption: it asserts that the precision of an agent's choices does not depend on whether probabilities are objectively given or subjectively inferred. If this assumption fails—as it plausibly could, since uncertain choices involve the additional cognitive step of forming beliefs about state probabilities—then the shared α could force utility parameters to absorb the misspecification.

This assumption is **testable**: systematic misfit in posterior predictive checks (e.g., under- or over-dispersion in one choice domain relative to the other) would suggest that separate sensitivity parameters are needed. [Report 7](07_generalizing_sensitivity.qmd) generalizes the model to allow distinct sensitivity parameters (α for uncertain, ω for risky choices) and examines the consequences of relaxing this assumption.
:::
...
## Why Risky Choices Directly Inform δ
...
This preference *directly* constrains υ₂ (and hence δ₁) without being mediated by subjective probabilities. More generally, by presenting lotteries that span the consequence space, we can triangulate the utility function with arbitrary precision (given sufficient data and lottery diversity).

## Study Design for m_1

To demonstrate that adding risky choices improves δ recovery, we need a controlled comparison between m_0 and m_1. Crucially, the **uncertain decision problems must be identical** between the two models—any differences in recovery should be attributable solely to the addition of risky choices.
...
Now we test this hypothesis. If known-probability risky choices provide a more direct utility signal, they should improve δ recovery relative to adding only more uncertain choices.
...
Before comparing recovery metrics, we verify that the MCMC sampling converged reliably across all conditions. Meaningful comparisons require that posterior summaries are trustworthy.

```{python}
#| label: mcmc-diagnostics
#| echo: false

def summarize_diagnostics(recovery_obj, label):
    """Summarize MCMC diagnostics across all parameter recovery iterations."""
    diag = recovery_obj.diagnostics if hasattr(recovery_obj, 'diagnostics') else None
    if diag is None:
        # Fall back to computing from posterior summaries
        summaries = recovery_obj.posterior_summaries_ if hasattr(recovery_obj, 'posterior_summaries_') else posterior_summaries_m1
        n_iter = len(summaries)
        rhat_ok = 0
        for s in summaries:
            if 'R_hat' in s.columns:
                rhat_ok += int(s['R_hat'].max() < 1.01)
            elif 'Rhat' in s.columns:
                rhat_ok += int(s['Rhat'].max() < 1.01)
            else:
                rhat_ok += 1  # Assume ok if not available
        print(f"{label}:")
        print(f"  Iterations with all R̂ < 1.01: {rhat_ok}/{n_iter} ({rhat_ok/n_iter:.0%})")
        return
    
    print(f"{label}:")
    print(f"  Iterations with all R̂ < 1.01: {diag.get('rhat_ok', 'N/A')}/{diag.get('total', 'N/A')}")
    print(f"  Iterations with divergent transitions: {diag.get('divergent', 'N/A')}")
    print(f"  Median minimum ESS: {diag.get('median_min_ess', 'N/A')}")

# Report diagnostics for m_1
print("=== MCMC Convergence Diagnostics ===\n")

# m_1 diagnostics
n_m1 = len(posterior_summaries_m1)
rhat_ok_m1 = sum(1 for s in posterior_summaries_m1 
                 if all(s.get('R_hat', s.get('Rhat', pd.Series([1.0]))).dropna() < 1.01))
print(f"m_1 (M=25 + N=25 risky):")
print(f"  Iterations with all R̂ < 1.01: {rhat_ok_m1}/{n_m1} ({rhat_ok_m1/n_m1:.0%})")
print(f"  Successful iterations: {n_m1}/50")
print()
```

::: {.callout-note}
## Diagnostic Note
All four conditions (m_0 at M=25, m_0 at M=50 with same alternatives, m_0 at M=50 with new alternatives, and m_1) are checked for convergence below when the full comparison is run. We report the fraction of iterations achieving R̂ < 1.01 for all parameters, and flag any iterations with divergent transitions. With 50 recovery iterations, coverage estimates have a standard error of approximately $\sqrt{0.9 \times 0.1 / 50} \approx 0.04$, so reported coverages should be interpreted with this uncertainty in mind.
:::
...
#| fig-cap: "Comparison of δ recovery between m_0 and m_1. Adding risky choices improves δ recovery in this design by narrowing credible intervals and reducing RMSE relative to the uncertain-only comparisons."
...
```{python}
#| label: mcmc-diagnostics-all
#| echo: false

# Report MCMC diagnostics for all four conditions
print("=== MCMC Convergence Diagnostics (All Conditions) ===\n")

def report_rhat(summaries, label):
    n = len(summaries)
    ok = 0
    for s in summaries:
        rhat_col = None
        for col in ['R_hat', 'Rhat', 'rhat']:
            if col in s.columns:
                rhat_col = col
                break
        if rhat_col is not None:
            ok += int(s[rhat_col].dropna().max() < 1.01)
        else:
            ok += 1
    print(f"{label}: {ok}/{n} iterations with all R̂ < 1.01 ({ok/n:.0%})")

report_rhat(posterior_summaries_m0, "m_0 (M=25)")
report_rhat(posterior_summaries_m0_m50, "m_0 (M=50 same alts)")
report_rhat(posterior_summaries_m0_m50_new, "m_0 (M=50 new alts)")
report_rhat(posterior_summaries_m1, "m_1 (M=25 + N=25)")
```
...
# Add relative improvement of m_1 over best m_0 condition
print("\n\nRelative RMSE Improvement (m_1 vs. best m_0 condition):")
for row in comparison_rows:
    param = row['Parameter']
    m0_vals = [float(row['m_0 (M=25)']), float(row['m_0 (M=50 same)']), float(row['m_0 (M=50 new)'])]
    best_m0 = min(m0_vals)
    m1_val = float(row['m_1'])
    if best_m0 > 0:
        pct = (best_m0 - m1_val) / best_m0 * 100
        print(f"  {param}: {pct:+.1f}% ({'improvement' if pct > 0 else 'comparable'})")
```
...
The four-way comparison above is evidence about the present model and simulation design, not a universal claim about all risky/uncertain choice experiments. The magnitude of the benefit from risky choices will depend on the diversity of lotteries, the shared-sensitivity assumption (relaxed in [Report 7](07_generalizing_sensitivity.qmd)), the informativeness of the uncertain alternatives, and the amount of data available. The comparison should also not be read as an empirical validation of the Anscombe–Aumann framework; it is a finite-sample recovery study of a specific SEU model.
:::
...
Here, ψ (from β) and υ (from δ) appear together in expected utilities. The two parameter blocks are structurally coupled, so information about δ must be extracted indirectly and accumulates more slowly than if a separate utility-only channel were available.
...
The extension from m_0 to m_1 follows a principled path motivated by the risk/uncertainty distinction and by the model's likelihood structure:
...
3. **The historical context**: Decision-theoretic treatments of risk and uncertainty suggest that known-probability choices can provide complementary information about utilities
...
::: {.callout-warning}
## Empirical update — see [Report 14](14_does_m1_identify_delta.qmd)
The four-way comparison sketched here was completed in [Report 14](14_does_m1_identify_delta.qmd) using a strictly matched-design recovery study (same true parameters, same study design across conditions, only the model + data slice varying). The finding is that **m_1's identification-theoretic advantage over m_0 on δ is small in practice** at this sample size and lottery design — roughly a 2% reduction in δ CI width at matched total choice count. The bulk of the m_1 advantage is on α recovery, not on δ. The structural identification argument below is correct, but decoupling β from δ in the likelihood does not, by itself, translate into a large δ-precision gain unless the lottery design and sample size are well chosen. Read Report 14 alongside this one for the empirical bottom line.
:::
...
If m_1 outperforms the expanded m_0, it supports the modeling strategy: risky choices provide a more direct route to utility identification because they bypass the subjective probability inference step.
...
::: {.callout-note}
## Terminology: "Generically"
Throughout this section, we say a property holds **generically** if the set of configurations where it fails has probability zero under any absolutely continuous distribution over the relevant space. In other words, failures require exact algebraic coincidences that are infinitely unlikely to arise in practice—any continuous randomization of the design avoids them.
:::
...
Consider model m_1 with $K \geq 2$ consequences and $D \geq 1$ feature dimensions, **under the shared-sensitivity specification** (a single $\alpha > 0$ governs both risky and uncertain choices). Let the class of decision problems include:
...
2. **Uncertain problems** with feature vectors from a set $\mathcal{W} \subseteq \mathbb{R}^D$ whose linear span is all of $\mathbb{R}^D$ (i.e., $\mathcal{W}$ contains at least $D$ linearly independent vectors)

Then the parameters $(\alpha, \delta, [\beta])$ are globally identifiable, where $[\beta]$ denotes the equivalence class of $K \times D$ matrices modulo additive row-common shifts $\beta_k \mapsto \beta_k + c$ (under which the softmax probabilities $\boldsymbol{\psi}(\beta, \mathbf{w})$ are invariant for every $\mathbf{w}$). Specifically, for any two configurations $\theta = (\alpha, \beta, \delta)$ and $\theta' = (\alpha', \beta', \delta')$ with $\alpha, \alpha' > 0$ that are not equal in the sense of inducing the same $(\alpha, \delta, [\beta])$, there exists a **binary choice problem** (with exactly two alternatives) where the choice probabilities under $\theta$ and $\theta'$ differ.
:::

Three remarks on this theorem are in order before we proceed to its proof.

::: {.callout-tip}
## Remark: Why the Equivalence Class on $\beta$
The raw matrix $\beta$ is not identifiable from choice data because adding the same vector $c \in \mathbb{R}^D$ to every row leaves $\boldsymbol{\beta}_k^\top \mathbf{w}$ shifted by the same scalar $c^\top \mathbf{w}$ for all $k$, and the softmax is invariant to a common shift of its logits. The theorem therefore identifies $\beta$ only up to this gauge. Equivalently, one could impose a gauge condition such as $\boldsymbol{\beta}_1 = \mathbf{0}$; under that constraint the remaining $(K-1) \times D$ entries of $\beta$ are globally identified.
:::

::: {.callout-tip}
## Remark: Validity for All $\alpha > 0$
The identification result holds for **any** positive sensitivity parameter $\alpha$, not merely in the limit $\alpha \to \infty$. The proof relies on the strict monotonicity of the logistic function $\sigma(t) = 1/(1 + e^{-t})$, which holds for all finite arguments. Since $\alpha > 0$ ensures the argument $\alpha(\eta_i - \eta_j) \neq 0$ whenever $\eta_i \neq \eta_j$, the softmax parameterization is globally injective on expected utility differences for any positive $\alpha$. No asymptotic arguments are invoked.
:::

::: {.callout-tip}
## Remark: Scope of the Shared-Sensitivity Specification
The theorem is stated for the shared-$\alpha$ version of m_1. If risky and uncertain choices have distinct sensitivity parameters, the same design logic applies but the parameter vector and proof require modification. [Report 7](07_generalizing_sensitivity.qmd) generalizes m_1 to allow distinct sensitivities (e.g., $\alpha$ for uncertain, $\omega$ for risky) and examines the consequences of relaxing this assumption.
:::

::: {.callout-tip}
## Remark: Binary Choice Sufficiency
The theorem guarantees that a **binary** choice problem (exactly two alternatives) suffices to distinguish any two parameter configurations. This is a strong result: identification does not require presenting large choice sets, only carefully chosen pairwise comparisons. For experimental design, this means that even a battery of simple two-alternative forced-choice trials can achieve full identification, provided the alternatives are sufficiently diverse.
:::

Before proving the theorem, we establish a key lemma.

```{python}
#| label: fig-affine-independence
#| fig-cap: "Affine independence on the 2-simplex (K=3). Left: Three probability vectors in general position form a non-degenerate triangle—they are affinely independent. Right: Three collinear probability vectors are affinely dependent and fail the identification condition."
#| echo: false

fig, axes = plt.subplots(1, 2, figsize=(12, 5))

def simplex_to_cartesian(p):
    """Convert a point on the 2-simplex to 2D Cartesian coordinates."""
    x = p[1] + 0.5 * p[2]
    y = (np.sqrt(3) / 2) * p[2]
    return x, y
...
for ax in axes:
    ...
    triangle_x = [simplex_to_cartesian(c)[0] for c in corners] + [simplex_to_cartesian(corners[0])[0]]
    triangle_y = [simplex_to_cartesian(c)[1] for c in corners] + [simplex_to_cartesian(corners[0])[1]]
    ax.plot(triangle_x, triangle_y, 'k-', linewidth=1.5, alpha=0.5)
    ax.fill(triangle_x, triangle_y, alpha=0.05, color='gray')
    
    # Label corners
    offsets = [(-0.06, -0.04), (0.03, -0.04), (0.03, 0.02)]
    for i, (corner, label) in enumerate(zip(corners, ['e₁', 'e₂', 'e₃'])):
        cx, cy = simplex_to_cartesian(corner)
        ax.annotate(label, (cx, cy), fontsize=11, fontweight='bold',
                   xytext=(offsets[i][0], offsets[i][1]), textcoords='offset fontsize')
    ax.set_aspect('equal')
    ax.axis('off')

# Left: affinely independent (general position)
ax = axes[0]
points_indep = np.array([[0.7, 0.2, 0.1], [0.15, 0.6, 0.25], [0.2, 0.15, 0.65]])
coords = [simplex_to_cartesian(p) for p in points_indep]
for i, (x, y) in enumerate(coords):
    ax.scatter(x, y, s=120, c='forestgreen', zorder=5, edgecolor='white', linewidth=1.5)
    ax.annotate(f'π⁽{i+1}⁾', (x, y), fontsize=10, xytext=(5, 5), textcoords='offset points')
# Draw triangle connecting points
tri_x = [c[0] for c in coords] + [coords[0][0]]
tri_y = [c[1] for c in coords] + [coords[0][1]]
ax.fill(tri_x, tri_y, alpha=0.15, color='forestgreen')
ax.plot(tri_x, tri_y, 'g--', linewidth=1.5, alpha=0.7)
ax.set_title('Affinely Independent\n(non-degenerate triangle)', fontsize=11, fontweight='bold')

# Right: affinely dependent (collinear)
ax = axes[1]
base = np.array([0.6, 0.3, 0.1])
direction = np.array([-0.4, 0.1, 0.3])
points_dep = np.array([base + t * direction for t in [0.0, 0.5, 1.0]])
coords_dep = [simplex_to_cartesian(p) for p in points_dep]
for i, (x, y) in enumerate(coords_dep):
    ax.scatter(x, y, s=120, c='crimson', zorder=5, edgecolor='white', linewidth=1.5)
    ax.annotate(f'π⁽{i+1}⁾', (x, y), fontsize=10, xytext=(5, 5), textcoords='offset points')
# Draw line through points
ax.plot([coords_dep[0][0], coords_dep[2][0]], [coords_dep[0][1], coords_dep[2][1]],
        'r--', linewidth=2, alpha=0.7)
ax.set_title('Affinely Dependent\n(collinear — identification fails)', fontsize=11, fontweight='bold')
...
*Sub-case 1a:* $\alpha = \alpha'$. At least one component $\eta^{(r)}_s \neq (\eta')^{(r)}_s$ differs. Picking any $t \neq s$ such that the differences also differ — which is generic since $\boldsymbol{\eta} \neq \boldsymbol{\eta}'$ and the differences cannot all coincide unless $\boldsymbol{\eta}$ and $\boldsymbol{\eta}'$ agree up to a common additive constant, contradicting their equality through the invertible $\Pi$ — the binary risky choice between $\boldsymbol{\pi}^{(s)}$ and $\boldsymbol{\pi}^{(t)}$ has probability $\sigma(\alpha(\eta^{(r)}_s - \eta^{(r)}_t))$, and strict monotonicity of $\sigma$ gives $\chi_1(\theta) \neq \chi_1(\theta')$.

*Sub-case 1b:* $\alpha \neq \alpha'$. Here we cannot rely on a single pairwise log-odds equation, because in principle the ratio $\alpha'/\alpha$ could rescale one specific utility difference $\eta^{(r)}_s - \eta^{(r)}_t$ to match $(\eta')^{(r)}_s - (\eta')^{(r)}_t$. We rule out global compensation by exploiting the normalization $\upsilon_1(\delta) = 0$ and $\upsilon_K(\delta) = 1$, which fixes the scale of $\boldsymbol{\upsilon}(\delta)$.

Consider the $K$ "corner" risky choice problems: for each $s = 2, \ldots, K$, the binary problem between $\boldsymbol{\pi}^{(s)}$ and $\boldsymbol{\pi}^{(1)}$ has log-odds $\alpha \cdot ((\boldsymbol{\pi}^{(s)} - \boldsymbol{\pi}^{(1)})^\top \boldsymbol{\upsilon}(\delta))$. Suppose, for contradiction, that $\theta$ and $\theta'$ produce identical choice probabilities in every binary risky problem drawn from the affinely independent set $\{\boldsymbol{\pi}^{(s)}\}$. Then for each $s \geq 2$:
$$
\alpha \cdot (\boldsymbol{\pi}^{(s)} - \boldsymbol{\pi}^{(1)})^\top \boldsymbol{\upsilon}(\delta) = \alpha' \cdot (\boldsymbol{\pi}^{(s)} - \boldsymbol{\pi}^{(1)})^\top \boldsymbol{\upsilon}(\delta')
$$
Writing $\boldsymbol{\Delta\pi}^{(s)} = \boldsymbol{\pi}^{(s)} - \boldsymbol{\pi}^{(1)}$, these $K-1$ vectors are linearly independent (affine independence of the $K$ original vectors is equivalent to linear independence of the $K-1$ pairwise differences). The $K-1$ equations therefore force $\alpha \boldsymbol{\upsilon}(\delta)$ and $\alpha' \boldsymbol{\upsilon}(\delta')$ to agree on every direction in the hyperplane $\{\mathbf{v} : \mathbf{1}^\top \mathbf{v} = 0\}$, i.e., they agree up to an additive scalar multiple of $\mathbf{1}$:
$$
\alpha \boldsymbol{\upsilon}(\delta) = \alpha' \boldsymbol{\upsilon}(\delta') + \lambda \mathbf{1}
$$
for some $\lambda \in \mathbb{R}$. Reading off the first coordinate ($\upsilon_1 = \upsilon'_1 = 0$) gives $\lambda = 0$. Reading off the last coordinate ($\upsilon_K = \upsilon'_K = 1$) then gives $\alpha = \alpha'$, contradicting $\alpha \neq \alpha'$. Therefore the supposition fails: at least one binary risky problem among $\{\boldsymbol{\pi}^{(s)}\}$ has $\chi_1(\theta) \neq \chi_1(\theta')$.

In both sub-cases, a binary risky problem distinguishes $\theta$ from $\theta'$, regardless of any difference in $\alpha$ or $\beta$.
...
*Case 3: $\delta = \delta'$, $\alpha = \alpha'$, but $[\beta] \neq [\beta']$ (i.e., $\beta$ and $\beta'$ differ by more than an additive row-common shift).*

By assumption, $\beta - \beta'$ has at least one row whose entries are not equal to the corresponding entries of a single common vector $c$; equivalently, the rows of $\beta - \beta'$ are not all identical. Hence there exists $\mathbf{w} \in \mathcal{W}$ such that $(\beta - \beta')\mathbf{w}$ is not a constant vector (i.e., not a scalar multiple of $\mathbf{1}$); the spanning condition on $\mathcal{W}$ guarantees such a $\mathbf{w}$.
...
**Combining the cases:** For any two configurations with distinct $(\alpha, \delta, [\beta])$, at least one of the three cases applies, guaranteeing the existence of a distinguishing decision problem. $\square$
...
3. The individual components $\delta$ and the gauge-class $[\beta]$ (i.e., $\beta$ modulo additive row-common shifts) are **weakly identified** through the composite mapping $(\beta, \delta) \mapsto \eta^{(u)}(\cdot)$. While generic point identification holds (distinct $(\delta, [\beta])$ pairs produce distinct expected utility functions except on a set of measure zero), information about these parameters accumulates more slowly from data compared to m_1, resulting in wider posterior credible intervals.
:::
...
$$
where $\psi_2 + \psi_3 = 1 - \psi_1$. The softmax probabilities $\psi_k$ depend on $\beta$ only through the differences $\boldsymbol{\beta}_k - \boldsymbol{\beta}_1$ (the softmax is invariant to adding a constant to all logits), so $\beta$ enters $\eta^{(u)}$ only through its gauge-class $[\beta]$. The effective free parameter space for determining $\psi$ therefore has dimension $(K-1) \times D = 2D$ rather than $KD$.
...
**Generic point identification holds:** For the equality $\psi_2(\beta, \mathbf{w}) \cdot \delta_1 + \psi_3(\beta, \mathbf{w}) = \psi_2(\beta', \mathbf{w}) \cdot \delta'_1 + \psi_3(\beta', \mathbf{w})$ to hold for all $\mathbf{w} \in \mathbb{R}^D$, we would need a precise functional relationship between $(\beta, \delta)$ and $(\beta', \delta')$ that holds across the entire feature space.

To see why this generically fails, consider the map $F: (\beta, \delta) \mapsto \eta^{(u)}(\cdot)$ that sends parameters to the expected utility function. The Jacobian of $F$ with respect to $(\beta, \delta)$ evaluated at any feature vector $\mathbf{w}$ has the block structure:
$$
\frac{\partial \eta^{(u)}}{\partial \delta_1}\bigg|_{\mathbf{w}} = \psi_2(\beta, \mathbf{w}), \qquad
\frac{\partial \eta^{(u)}}{\partial \beta_{kd}}\bigg|_{\mathbf{w}} = \delta_1 \frac{\partial \psi_2}{\partial \beta_{kd}}\bigg|_{\mathbf{w}} + \frac{\partial \psi_3}{\partial \beta_{kd}}\bigg|_{\mathbf{w}}
$$
The $\delta_1$-derivative is $\psi_2(\beta, \mathbf{w})$, which is strictly positive. Thus perturbations in $\delta_1$ always change $\eta^{(u)}$. Similarly, for generic $\beta$, the $\beta$-derivatives do not vanish across all $\mathbf{w}$ simultaneously. The set of parameter values $(\beta, \delta)$ where the Jacobian drops rank—allowing distinct parameters to produce the same function—is a proper algebraic subvariety of the parameter space, hence has Lebesgue measure zero. This is a standard argument in the generic identification literature for models with analytic parameterizations [see, e.g., @rothenberg1971identification].
...
- **Practical consequence:** The weak identification of $\delta$ in m_0 manifests as slower posterior concentration—wider credible intervals that shrink more slowly with sample size compared to m_1. This matches our empirical parameter recovery findings. Note that this is a *rate* phenomenon, not a fundamental non-identification: under the generic identification and regularity conditions described above, m_0 posteriors would still be expected to concentrate with sufficient data.
