/**
 * Bayesian Decision Theory Model (m_1) for Simulation Based Calibration
 * 
...
transformed data {
  ...
  // Generate choices for uncertain problems
  ...
  // Generate choices for risky problems
  array[N] int<lower=1> z;
  {
    int pos = 1;
    for (n in 1:N) {
      vector[N_risky[n]] problem_eta = segment(eta_risky_, pos, N_risky[n]);
      vector[N_risky[n]] choice_probs = softmax(alpha_ * problem_eta);
  ...
  // Likelihood
  {
    ...
    // Likelihood for risky choices
    {
      ...
      for (n in 1:N) {
        ...
        vector[N_risky[n]] choice_probs = softmax(alpha * problem_eta);
