...
This report bridges theory and practice by documenting the concrete implementation in Stan. We show precisely how the abstract formulation is instantiated, highlighting the specific choices made for:
...
Recall the key quantities from the abstract formulation:

| Abstract Notation | Description |
|-------------------|-------------|
| $\mathcal{R} = \{1, \ldots, R\}$ | Set of distinct alternatives |
| $K$ | Number of possible consequences |
| $\boldsymbol{\psi}_r \in \Delta^{K-1}$ | Subjective probabilities for alternative $r$ |
| $\boldsymbol{\upsilon} \in [0,1]^K$ | Ordered utility vector |
| $\eta_r = \boldsymbol{\psi}_r^\top \boldsymbol{\upsilon}$ | Expected utility of alternative $r$ |
| $\alpha \geq 0$ | Sensitivity parameter |

The abstract formulation left several things unspecified:
...
The m_0 model provides concrete answers to each of these questions.
...
::: {.callout-note}
## Notation Mapping
| Stan Variable | Abstract Notation | Description |
|---------------|-------------------|-------------|
| `M` | $M$ | Number of decision problems |
| `K` | $K$ | Number of consequences |
| `D` | $D$ | Feature dimensions |
| `R` | $R$ | Number of distinct alternatives |
| `w[r]` | $\mathbf{w}_r$ | Feature vector for alternative $r$ |
| `I[m,r]` | $I_{m,r}$ | Indicator for alternative availability |
| `y[m]` | $y_m$ | Observed choice in problem $m$ |
:::
...
The parameter `alpha` is constrained to be non-negative, matching the theoretical requirement $\alpha \geq 0$. Its interpretation was established in Report 1: with utilities normalized to $[0,1]$, $\alpha$ represents the log-odds change per unit of expected utility difference.
...
for i in range(n_samples):
    ...
    # Plot delta
    axes[0].bar([f'δ₁ (sample {i+1})', f'δ₂ (sample {i+1})'], delta, 
                alpha=0.7, color=colors[i], label=f'Sample {i+1}')
...
The model includes generated quantities for model checking and comparison:

```{.stan filename="m_0.stan (generated quantities)"}
generated quantities {
  // Log-likelihood for model comparison (e.g., LOO-CV)
  vector[M] log_lik;
  for (i in 1:M) {
...
- **`log_lik`**: Point-wise log-likelihoods for leave-one-out cross-validation
- **`y_pred`**: Posterior predictive samples for checking model fit

## The Simulation Model (m_0_sim.stan)

For prior predictive analysis and parameter recovery studies, we use a companion model that generates synthetic data:

```{.stan filename="m_0_sim.stan (key excerpts)"}
transformed data {
...
generated quantities {
  ...
  // ... (computation of psi, upsilon, eta, choice_probs) ...
  
  for (i in 1:M) {
    y[i] = categorical_rng(choice_probs);
    
    // Track whether SEU maximizer was selected
...
The simulation model additionally tracks `selected_seu_max`—whether the SEU-maximizing alternative was chosen in each problem. This is useful for prior predictive analysis of "rationality" under the prior.
